...
**Completed summary (Settings company save guard, 2026-09-16):** Updated the Settings company-name flow so unchanged values no longer trigger `PATCH /auth/tenant` or show a success toast. The Save button is disabled until the trimmed company name actually differs from the saved tenant name, and the handler itself now no-ops on unchanged input as a second guard.
**Completed summary (Blur validation UX fix, 2026-09-16):** Updated the shared `useFormFields` hook so fields are only marked touched on blur after meaningful interaction (typing or a non-empty DOM value), while submit-time and server-side validation behavior remain unchanged. This prevents immediate required-field errors after an empty focus-then-blur across sign-in, sign-up, forgot-password, reset-password, join, and settings password forms.
**Completed summary (Auth form autofocus UX fix, 2026-09-15):** Removed forced initial focus from the sign-in, sign-up, forgot-password, and reset-password pages so untouched fields no longer show blur-triggered validation errors when users click elsewhere on the page. Verified no remaining auth-page `autoFocus` usage; shared submit/server-error focus behavior remains intact.
